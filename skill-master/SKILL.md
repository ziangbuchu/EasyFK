...
4. **通读整份 SKILL.md 做一次自洽检查**（标准见 §5），把违反点列出来给用户
...
## 5. 自洽检查（review skill 的统一标准）

用户说"帮我看一下这个 skill 逻辑恰不恰当"、"自洽检查"、"通读一下"、"读着别扭"、"顺一下"——指向的都是同一件事：让整份 skill 文档作为一个系统读起来不别扭。

下面 5 条维度同时成立才算过：

1. **前后一致**：同一个概念在不同地方用同一个名字、归到同一个位置；同一条规则在不同地方说出来意思不变。不出现"A 处说放第 1 节、B 处说放第 3 节"这种矛盾，也不出现"上面叫'目标文件状况'、下面叫'着陆区'"这种术语漂移。
2. **完备无空洞**：读者能想到的边角情况都有交代。读到一条规则不会在心里嘀咕"那 X 情况怎么办"——要么明说处理方式，要么明说"这种情况这里不管"。退出条件里出现的要求，正文或 reference 里要有出处，不能空降。
3. **比例恰当**：重要的说得重、有理由；次要的说得轻、不喧宾夺主。不出现"这条为什么这么严 / 这条为什么这么糊"的违和感。一段比另一段多三倍篇幅，应该是因为它真的多三倍重要，不是因为顺手写多了。
4. **顺序自然**：从上到下读就能建立理解，不需要先跳到后文才能看懂前文，也不需要回头翻才能接住当前这句。每节出现的位置匹配读者此刻已经积累的上下文——心智模型在前，操作流程在中，模板和退出条件在后。
5. **无残留疑问**：读完任意一段不冒出"这跟刚才那条不是冲突吗"、"这一句怎么突然冒出来"、"这里是不是漏了一步"。

**怎么做这次检查：**

通读整份 SKILL.md（必要时连 reference 一起），按上面 5 条逐条扫，违反点按下面三档分级输出，每条都标位置（行号或节标题）：

- **必须修**——影响理解或会让 AI 走错的硬伤（违反维度 1、2、5 的实质性矛盾或缺失）
- **建议修**——读着别扭但还能猜出意思的（多数维度 3、4 问题）
- **不用改但提一下**——审美问题或风格选择，留给用户决定

列完让用户选改哪些，不要直接动手。

**何时主动触发：**

- 用户明示"自洽检查 / 通读 / 顺一下 / 看下逻辑恰不恰当"
- §2.6 自测的最后一步——交付新 skill 前自己跑一次
- §3 优化已有 skill 时，定位问题的第一道扫描就是这个
